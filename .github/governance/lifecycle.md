...
## 7. Class A / Class B Lifecycle Distinction

Class A (Primary) tickets follow the full 9-state machine above.
Class B (Background) tickets spawned by the Operational Concurrency Floor
(OCF) follow a simplified 5-state lifecycle. Full OCF spec:
`governance/concurrency_floor.md`.

### Class B Lifecycle States

| State | Description | Owner |
|-------|-------------|-------|
| **BG-READY** | Background ticket generated by scheduler | System |
| **BG-IMPLEMENTING** | Background worker analyzing codebase | BG Worker |
| **BG-REVIEW** | ReaperOAK reviews finding quality | ReaperOAK |
| **BG-COMMIT** | Finding report committed | ReaperOAK |
| **BG-DONE** | Background ticket complete, worker terminated | System |

### Class B Transition Rules

| From | To | Trigger | Guard |
|------|----|---------|-------|
| BG-READY | BG-IMPLEMENTING | Worker spawned, no conflict with Class A | No Class A file overlap |
| BG-IMPLEMENTING | BG-REVIEW | Worker emits BG_WORKER_COMPLETED | finding_summary present |
| BG-IMPLEMENTING | BG-DONE | Preempted by Class A arrival | BG_WORKER_PREEMPTED event |
| BG-REVIEW | BG-COMMIT | Finding is actionable | ReaperOAK approves |
| BG-REVIEW | BG-DONE | Finding is not actionable | Low-value finding discarded |
| BG-COMMIT | BG-DONE | Commit succeeds | `[BG-<TYPE>]` format |

### Class B Exclusions

Class B tickets do NOT run the full post-execution chain. Specifically:
- No QA Engineer review (BG work is read-only analysis)
- No Validator DoD check (BG output is reports, not implementation)
- No Documentation Specialist step (BG findings are self-contained)
- No CI Reviewer step (BG reports don't affect CI)
- Commit uses `[BG-<TYPE>]` format, not `[TICKET-ID]` format

### Preemption Handling

Class A tickets ALWAYS preempt Class B. When a new Class A ticket arrives
and all workers are busy, the lowest-priority Class B worker is paused
and its capacity reassigned. The paused BG ticket returns to BG-READY
queue (resumable). See `concurrency_floor.md` §5 for preemption rules.
