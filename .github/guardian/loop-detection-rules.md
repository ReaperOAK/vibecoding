...
- ESCALATE: Notify Ticketer with loop evidence (tool name, args, step numbers)
...
- ESCALATE: Notify Ticketer for re-scoping or alternative approach

### Signal: Retry Exhaustion
- DETECT: Task retried 3 times with same failure class
- ACTION: Set task status to BLOCKED, stop retries
- ESCALATE: Notify Ticketer → human escalation if unresolvable
...
- ESCALATE: Ticketer re-scopes or reassigns task; notify user if unresolvable
...
- ESCALATE: Ticketer attempts to unblock root; if blocked > 1 full cycle → user notification
...
- ESCALATE: Notify Ticketer with evidence of attempted skip; Ticketer re-delegates with explicit state requirements and mandatory chain checks
...
- ESCALATE: Notify Ticketer; if repeated (≥ 2 advancement attempts with failing DoD), flag the owning agent for re-delegation or user intervention
...
- ESCALATE: Notify Ticketer to re-delegate with explicit initialization instructions; if module scaffolding cannot be completed after 2 attempts → user notification
...
- ESCALATE: Notify Ticketer; require UIDesigner ticket completion before unblocking. Override requires explicit user approval logged in `decisionLog.md`
...
- ESCALATE: Notify Ticketer → present full rejection history (all 3 rejection reports) to user with options: force-approve (override logged in `decisionLog.md`), cancel task, or reassign to a different agent
...
| >100% | VIOLATION: Task killed, escalate to Ticketer |

### Session-Level Budgets
- Total session budget tracked across all agents
- Ticketer monitors aggregate burn rate
...
- Only Ticketer or human may set/clear STOP_ALL
