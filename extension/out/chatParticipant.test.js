...
jest.mock('vscode', () => ({
    ...
    chat: {
        createChatParticipant: (...args) => mockCreateChatParticipant.apply(null, args)
    }
}), { virtual: true });
...
function createSpawnProcess(exitCode, stdout = '', stderr = '') {
    const processEmitter = new events_1.EventEmitter();
    processEmitter.stdout = new events_1.EventEmitter();
    processEmitter.stderr = new events_1.EventEmitter();
    setImmediate(() => {
        if (stdout) {
...
describe('VibecodingParticipant', () => {
    ...
    it('formats status dashboard from valid JSON output', async () => {
        mockSpawn.mockReturnValue(createSpawnProcess(0, JSON.stringify({ summary: { READY: 2, BACKEND: 1 } })));
    ...
    it('handles slash command via request.command field', async () => {
        mockSpawn.mockReturnValue(createSpawnProcess(0, JSON.stringify({ summary: { READY: 1 } })));
        const participant = chatParticipant_1.VibecodingParticipant.create();
        const markdown = jest.fn();
        await participant.handleChatRequest({
...
describe('VibecodingParticipant additional coverage', () => {
    ...
    it('exposes and clears singleton instance', () => {
        chatParticipant_1.VibecodingParticipant.create();
        expect(chatParticipant_1.VibecodingParticipant.getInstance()).not.toBeNull();
        chatParticipant_1.VibecodingParticipant.disposeInstance();
        expect(chatParticipant_1.VibecodingParticipant.getInstance()).toBeNull();
    });
});
//# sourceMappingURL=chatParticipant.test.js.map